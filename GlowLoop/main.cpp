...
// ---------------------------------------------------------------------------
// Input helpers
// Small console I/O utilities shared by every menu action below: normalizing
// text for case-insensitive comparisons, and reading validated integers and
// non-blank lines from the user.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Routine setup
// Functions that build the two SkincareRoutine lists and let the rest of
// the program pick which one (Morning or Night) a menu action applies to.
// ---------------------------------------------------------------------------
...
void loadStarterRoutines(SkincareRoutine &morningRoutine,
  ...
  morningRoutine.add(SkincareStep("Cleanser", "Gentle Facial Cleanser",
                                  "1-2 pumps", "Morning", 1, "Cleansing",
                                  "Massage gently onto wet skin, then rinse."));
  morningRoutine.add(SkincareStep("Serum", "Vitamin C Serum", "2-3 drops",
                                  "Morning", 2, "Brightening",
                                  "Apply evenly to clean, dry skin."));
  morningRoutine.add(SkincareStep("Moisturizer", "Daily Moisturizer",
                                  "Pea-sized amount", "Morning", 3, "Hydration",
  ...
  nightRoutine.add(SkincareStep("Cleanser", "Gentle Facial Cleanser",
                                "1-2 pumps", "Night", 1, "Cleansing",
                                "Massage gently onto wet skin, then rinse."));
  ...
  nightRoutine.add(SkincareStep(
      "Eye Cream", "Hydrating Eye Cream", "Rice-grain amount per eye", "Night",
      3, "Under-eye hydration",
      "Pat gently around each eye using a ring finger."));
  nightRoutine.add(SkincareStep(
...
// ---------------------------------------------------------------------------
// Menu actions
// One function per main-menu option: adding, removing, searching, and
// displaying skincare steps, plus walking a routine step by step. Each
// reads whatever input it needs and reports the outcome to the console.
// ---------------------------------------------------------------------------
...
void addSkincareStep(SkincareRoutine &morningRoutine,
  ...
  SkincareStep newStep(stepName, productName, amountToUse, timeOfDay, stepOrder,
...
void removeSkincareStep(SkincareRoutine &morningRoutine,
  ...
  bool removed =
      selectedRoutine.removeIf([&searchText](const SkincareStep &step) {
...
void startRoutine(const SkincareRoutine &routine,
  ...
  while (true) {
    ...
    std::cout << "\n" << routineName << " ROUTINE\n\nCurrent Step:\n\n";
    currentStep->displayStep(static_cast<int>(currentPosition) + 1);
    std::cout << "\n1. Next Step\n2. End Routine\n\n";
...
// ---------------------------------------------------------------------------
// Program entry point
// The menu display and the main loop that reads a choice and dispatches to
// the matching menu action until the user exits.
// ---------------------------------------------------------------------------
